...
from duckduckgo_search import DDGS  # Import the updated DDGS class

def getDiseaseInfo(keywords):
    search = DDGS()  # Create an instance of DDGS

    try:
        # Perform the search with desired parameters
        results = search.text(keywords, region='wt-wt', safesearch='Off', time='y')

        if not results:  # Check if any results were found
            raise Exception("No search results found for the disease.")

        return results[0]['body']  # Return the body of the first result
